...
open class FBUser : FBObject("_User") {
     ...
     */
    fun signUpByEmail(emailCode: String): FBCall<FBUser> = fbCall {
     ...
     * @return FBCall<FBUser> 可链式调用 onSuccess/onFailure
     */
    fun signUpBySms(smsCode: String): FBCall<FBUser> = fbCall {
     ...
     * @return FBCall<FBUser> 可链式调用 onSuccess/onFailure
     */
    fun signUpByUsername(): FBCall<FBUser> = fbCall {
    ...
    internal fun applyAuthResponse(res: Map<String, Any?>) {
        val token = res["access_token"]?.toString()
            ?: throw FreelyBaseException("认证响应缺少 access_token")
    ...
    companion object {
        ...
        fun sendEmailCode(email: String): FBCall<String> = fbCall {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/password/send-code",
        ...
        fun verifyEmailCode(email: String, emailCode: String): FBCall<Boolean> = fbCall {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/email-code/verify",
        ...
        fun sendSmsCode(phone: String): FBCall<String> = fbCall {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/sms/send-code",
        ...
        fun verifySmsCode(phone: String, smsCode: String): FBCall<Boolean> = fbCall {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/sms/verify",
        ...
        fun loginWithAuthDataAs(snsType: String, accessToken: String, expiresIn: String, userId: String): FBCall<FBUser> =
            fbCall { loginWithAuthDataInternal(FBUser::class.java, snsType, accessToken, expiresIn, userId) }
         ...
         * @param T 自定义用户类型
         */
        inline fun <reified T : FBUser> loginWithAuthDataAs(
        ...
        @PublishedApi
        internal suspend fun <T : FBUser> loginWithAuthDataInternal(
            clazz: Class<T>, snsType: String, accessToken: String, expiresIn: String, userId: String
        ): T {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/third-party/login",
        ...
        fun associateWithAuthData(snsType: String, accessToken: String, expiresIn: String, userId: String): FBCall<Unit> =
            fbCall {
                HttpClient.post(
                    "/public/apps/${FreelyBase.appId}/auth/third-party/associate",
                    mapOf("sns_type" to snsType, "access_token" to accessToken, "expires_in" to expiresIn, "user_id" to userId)
                )
            }
        ...
        fun dissociateAuthData(snsType: String): FBCall<Unit> = fbCall {
            HttpClient.delete("/public/apps/${FreelyBase.appId}/auth/third-party/$snsType")
        }
        ...
        fun login(account: String, password: String): FBCall<FBUser> =
            fbCall { loginAsInternal(FBUser::class.java, account, password) }
         ...
         * @param T 自定义用户类型
         */
        inline fun <reified T : FBUser> loginAs(account: String, password: String): FBCall<T> =
            fbCall { loginAsInternal(T::class.java, account, password) }

        @PublishedApi
        internal suspend fun <T : FBUser> loginAsInternal(clazz: Class<T>, account: String, password: String): T {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/login",
        ...
        fun loginWithEmailCode(email: String, emailCode: String): FBCall<FBUser> =
            fbCall { loginWithEmailCodeInternal(FBUser::class.java, email, emailCode) }

        /**
         * 邮箱验证码登录（泛型版本）。
         */
        inline fun <reified T : FBUser> loginWithEmailCodeAs(email: String, emailCode: String): FBCall<T> =
            fbCall { loginWithEmailCodeInternal(T::class.java, email, emailCode) }

        @PublishedApi
        internal suspend fun <T : FBUser> loginWithEmailCodeInternal(clazz: Class<T>, email: String, emailCode: String): T {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/login/email-code",
                mapOf("email" to email, "email_code" to emailCode)
            )
            return parseAuthAs(clazz, res)
        }
        ...
        fun loginWithSmsCode(phone: String, smsCode: String): FBCall<FBUser> =
            fbCall { loginWithSmsCodeInternal(FBUser::class.java, phone, smsCode) }

        /**
         * 短信验证码登录（泛型版本）。
         */
        inline fun <reified T : FBUser> loginWithSmsCodeAs(phone: String, smsCode: String): FBCall<T> =
            fbCall { loginWithSmsCodeInternal(T::class.java, phone, smsCode) }

        @PublishedApi
        internal suspend fun <T : FBUser> loginWithSmsCodeInternal(clazz: Class<T>, phone: String, smsCode: String): T {
            val res = HttpClient.post(
                "/public/apps/${FreelyBase.appId}/auth/sms/login",
                mapOf("phone" to phone, "sms_code" to smsCode)
            )
            return parseAuthAs(clazz, res)
        }
         ...
         */
        @JvmName("currentUser")
        fun currentUser(): FBCall<FBUser> = fbCall { currentUserInternal(FBUser::class.java) }

        /**
         * 获取当前登录用户信息（泛型版本）。
         */
        inline fun <reified T : FBUser> currentUserAs(): FBCall<T> = fbCall { currentUserInternal(T::class.java) }

        @PublishedApi
        internal suspend fun <T : FBUser> currentUserInternal(clazz: Class<T>): T {
            val res = HttpClient.get("/public/apps/${FreelyBase.appId}/auth/me")
            ...
            if (clazz != FBUser::class.java && user.objectId.isNotBlank()) {
                runCatching {
                    val dataRes = HttpClient.get("/public/apps/${FreelyBase.appId}/classes/_User/${user.objectId}")
         ...
         */
        fun restoreCurrentUser(): FBUser? = restoreCurrentUserAs(FBUser::class.java)

        /**
         * 从本地缓存恢复用户信息（泛型版本）。
         */
        inline fun <reified T : FBUser> restoreCurrentUserAs(): T? = restoreCurrentUserAs(T::class.java)

        fun <T : FBUser> restoreCurrentUserAs(clazz: Class<T>): T? {
        ...
        @PublishedApi
        @Suppress("UNCHECKED_CAST")
        internal fun <T : FBUser> parseAuthAs(clazz: Class<T>, res: Map<String, Any?>): T {
            val token = res["access_token"]?.toString()
                ?: throw FreelyBaseException("认证响应缺少 access_token")
            FreelyBase.setToken(token)
            val userMap = res["user"] as? Map<String, Any?> ?: emptyMap()
