 ...
 * ```
 */
class FBQuery<T : FreelyObject>(private val clazz: Class<T>) {
    ...
    /** 字段匹配正则表达式 */
    fun matches(field: String, regex: String)     = and(field, "regex", regex)
    ...
    /**
     * 查询距离指定地理位置最近的对象（按距离升序排序）。
     *
     * @param field 地理位置字段名
     * @param point 参考地理位置
     * @return 当前查询对象
     */
    fun near(field: String, point: FBGeoPoint): FBQuery<T> {
        and(field, "near", point.toMap())
        return this
    }
     ...
     * @return 当前查询对象
     */
    fun withinKilometers(field: String, point: FBGeoPoint, maxDistance: Double): FBQuery<T> {
        and(field, "within_km", mapOf(
     ...
     * @param field 地理位置字段名
     * @param point 中心地理位置
     * @param maxDistance 最大距离（英里）
     * @return 当前查询对象
     */
    fun withinMiles(field: String, point: FBGeoPoint, maxDistance: Double): FBQuery<T> {
        and(field, "within_miles", mapOf(
            "point" to point.toMap(),
            "distance" to maxDistance
        ))
        return this
     ...
     * @return 当前查询对象
     */
    fun withinGeoBox(field: String, southwest: FBGeoPoint, northeast: FBGeoPoint): FBQuery<T> {
        and(field, "within_box", mapOf(
     ...
     */
    suspend fun <S : FreelyObject> matchesQuery(field: String, subQuery: FBQuery<S>): FBQuery<T> {
     ...
     */
    suspend fun <S : FreelyObject> doesNotMatchQuery(field: String, subQuery: FBQuery<S>): FBQuery<T> {
        val ids = subQuery.select("objectId").findInternal().map { it.objectId }.filter { it.isNotBlank() }
     ...
     * 按字段排序。
     * 
     * @param field 字段名，使用 "-" 前缀表示降序，如 "-createdAt"
     */
    fun order(field: String): FBQuery<T> { 
        orderByField = field
        return this 
    }
...
inline fun <reified T : FreelyObject> FBQuery(): FBQuery<T> = FBQuery(T::class.java)
