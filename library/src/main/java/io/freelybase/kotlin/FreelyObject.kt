...
abstract class FreelyObject(val tableName: String) {
    ...
    private val appKey get() = FreelyBase.appKey
     ...
     * - FBFile 字段 -> path 字符串
     * - FBGeoPoint 字段 -> {"latitude": ..., "longitude": ...}
    ...
    internal fun toDataMap(): Map<String, Any?> {
        ...
        collectFields(this::class.java).forEach { field ->
            ...
            map[field.name] = when {
                ...
                value is FBGeoPoint -> value.toMap()
                field.isAnnotationPresent(Pointer::class.java) && value is FreelyObject ->
                    value.objectId.ifBlank { null }
                field.isAnnotationPresent(Relation::class.java) && value is List<*> ->
                    value.filterIsInstance<FreelyObject>().map { it.objectId }.filter { it.isNotBlank() }
                else -> value
            }
        }
        return map
    ...
    internal fun fillFromResponse(res: Map<String, Any?>) {
        ...
        collectFields(this::class.java).forEach { field ->
            ...
            if (field.isAnnotationPresent(Pointer::class.java)) {
                ...
                when {
                    // 后端返回了展开的对象 map
                    rawValue is Map<*, *> && FreelyObject::class.java.isAssignableFrom(fieldType) -> {
                        runCatching {
                            val obj = fieldType.getDeclaredConstructor().newInstance() as FreelyObject
                            val wrapped = mapOf(
                    ...
                    // 后端返回了 objectId 字符串
                    rawValue is String && FreelyObject::class.java.isAssignableFrom(fieldType) -> {
                        runCatching {
                            val existing = field.get(this) as? FreelyObject
                            if (existing != null) {
            ...
            if (field.isAnnotationPresent(Relation::class.java)) {
                ...
                val itemClass = runCatching {
                    ...
                    (typeArg as? Class<*>)?.takeIf { FreelyObject::class.java.isAssignableFrom(it) }
                }.getOrNull() ?: return@forEach

                val list = mutableListOf<FreelyObject>()
                when {
                    rawValue is List<*> && rawValue.firstOrNull() is Map<*, *> -> {
                        rawValue.filterIsInstance<Map<*, *>>().forEach { itemMap ->
                            runCatching {
                                val obj = itemClass.getDeclaredConstructor().newInstance() as FreelyObject
                                val wrapped = mapOf(
            ...
            // FBGeoPoint 字段
            if (field.type == FBGeoPoint::class.java) {
                runCatching {
                    when (rawValue) {
                        is Map<*, *> -> {
                            val geoPoint = FBGeoPoint.fromMap(rawValue)
                            if (geoPoint != null) field.set(this, geoPoint)
                        }
                        is String -> {
                            val geoPoint = FBGeoPoint.fromString(rawValue)
                            if (geoPoint != null) field.set(this, geoPoint)
                        }
                    }
                }
    ...
    private fun collectFields(clazz: Class<*>): List<java.lang.reflect.Field> {
        ...
        var c: Class<*>? = clazz
        while (c != null && c != FreelyObject::class.java) {
     ...
     */
    inline fun <reified T : FreelyObject> save(): FBCall<T> = saveInternal()
     ...
     * @return FBCall<T> 可链式调用 onSuccess/onFailure
     */
    inline fun <reified T : FreelyObject> fetch(): FBCall<T> = fetchInternal()
     ...
     * @return FBCall<T> 可链式调用 onSuccess/onFailure
     */
    inline fun <reified T : FreelyObject> update(): FBCall<T> = updateInternal()
    ...
    fun delete(): FBCall<Unit> = fbCall {
        ...
        HttpClient.delete("/public/apps/$appKey/classes/$tableName/$objectId")
        objectId = ""
    }

    // ── 内部实现方法（非 inline，可以访问 internal/private 成员）──────────────

    @PublishedApi
    internal fun <T : FreelyObject> saveInternal(): FBCall<T> = fbCall {
        val res = HttpClient.post("/public/apps/$appKey/classes/$tableName", mapOf("data" to toDataMap()))
    ...
    @PublishedApi
    internal fun <T : FreelyObject> fetchInternal(): FBCall<T> = fbCall {
        require(objectId.isNotBlank()) { "objectId 为空，请先调用 save() 或通过查询获取对象" }
        val res = HttpClient.get("/public/apps/$appKey/classes/$tableName/$objectId")
        fillFromResponse(res)
        @Suppress("UNCHECKED_CAST")
        this as T
    }
    ...
    @PublishedApi
    internal fun <T : FreelyObject> updateInternal(): FBCall<T> = fbCall {
        require(objectId.isNotBlank()) { "objectId 为空，请先调用 save() 或通过查询获取对象" }
        val res = HttpClient.put("/public/apps/$appKey/classes/$tableName/$objectId", mapOf("data" to toDataMap()))
        fillFromResponse(res)
        @Suppress("UNCHECKED_CAST")
        this as T
    }
