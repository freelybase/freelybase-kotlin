...
object FreelyBase {
    ...
    fun initialize(
        ...
        android.util.Log.d("FreelyBase", "initialize: restored token=${token?.take(20)}")

        if (enableEncryption) {
            appScope.launch(Dispatchers.IO) {
                runCatching { CryptoManager.fetchPublicKey() }
            }
        }
    }
    ...
    fun setToken(token: String?) {
        this.token = token
        android.util.Log.d("FreelyBase", "setToken: ${token?.take(20)}")
        if (::prefs.isInitialized) {
            val editor = prefs.edit()
            if (token != null) editor.putString(KEY_TOKEN, token) else editor.remove(KEY_TOKEN)
            editor.apply()
        } else {
            android.util.Log.w("FreelyBase", "setToken: prefs not initialized!")
        }
    }
    ...
    fun getServerTime(): FBCall<String> = fbCall {
        ...
        res["server_time"]?.toString()
            ?: throw FreelyBaseException("获取服务器时间失败")
    }
